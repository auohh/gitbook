...
# 配置
# 默认根目录为 ./gitbook，可通过命令行参数 --dir 覆盖
MARKDOWN_ROOT = './gitbook'  # 搜索的根目录
IMG_DIR = os.path.join(MARKDOWN_ROOT, 'assets', 'images')  # 图片保存目录（位于根目录下）
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Download external images referenced in markdown files and rewrite links')
    parser.add_argument('--dir', '-d', default='./gitbook', help="Markdown root directory (default: ./gitbook)")
    args = parser.parse_args()

    # 更新全局变量以便脚本在指定目录下工作
    MARKDOWN_ROOT = args.dir
    IMG_DIR = os.path.join(MARKDOWN_ROOT, 'assets', 'images')
